...
def is_hangul(c):
    return (
        ...
        or is_hangul_jamo(c)
        or is_hangul_compat_jamo(c)
        or is_hangul_jamo_exta(c)
        or is_hangul_jamo_extb(c)
    )
...
def split_syllable_char(c):
    ...
    return tuple(
        ...
        for pos, idx in zip([INITIAL, MEDIAL, FINAL], [init, med, final])
    )
    ...
    def try_split(c):
        try:
            ...
            if ignore_err:
                return (c,)
            raise ValueError(
                f"encountered an unsupported character: " f"{c} (0x{ord(c):x})"
            )
